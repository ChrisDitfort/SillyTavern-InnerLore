 ...
 *   node setup.cjs [path-to-sillytavern]
 *
 * Named .cjs because newer SillyTavern installs set "type": "module" in
 * their root package.json, which makes Node parse plain .js files as ES
 * modules where require() is unavailable.
 *
...
async function main() {
    ...
    if (!stRoot) {
        die([
            ...
            'or pass the path:  node setup.cjs /path/to/SillyTavern',
        ].join('\n'));
    }
